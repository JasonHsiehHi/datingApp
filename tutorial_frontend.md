...
常看到的SyntaxError()物件就是繼承Error()而來
屬於文法錯誤導致JS無法正確解讀 如果拋出錯誤會中斷之後JS所又的程序
...
因為做運算只考慮到整數的部分 故： 5.2442|0 等同為 5|0 且任何數與0做OR都為自己 且只會針對小數點錢的部分做處理 故結果為101
& 換成二進位後當兩個數同為1時才為1 , | 換成二進位後只需其中為1即為1
